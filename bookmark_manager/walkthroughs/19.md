...
Signing out is, in CRUD terms, **'destroying' a session resource**. With that in mind, let's implement a test that clicks a button to sign out on the links page:
...
We can implement this button, which destroys a session, similarly to how we implemented the button that destroys links:
