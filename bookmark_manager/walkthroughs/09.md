...
- Visiting a page, `/add-a-new-bookmark`
...
> Dislike this route structure? Think `add-a-new-bookmark` is a crummy name for a route? I do too! We'll fix it later, to make it RESTful.

**I run my test.** I have a new error: there's nothing to 'fill in' on the page. So, I'll add a form to the `views/add_a_new_bookmark.erb` file:

```html
<!-- inside views/add_a_new_bookmark.erb -->
...
- The user visits the `/add-a-new-bookmark` route and `GET`s an HTML page containing a form from the Sinatra application.
- The user fills out the form and submits.
- Submitting a form defaults to sending a `GET` request to the current URL: `/add-a-new-bookmark`. 
...
```html
<form action="/create-new-bookmark" method="post">
  <input type="text" name="url" />
  <input type="submit" value="Submit" />
</form>
```
...
Failures:
  ...
  1) Adding a new bookmark A user can add a bookmark to Bookmark Manager
     Failure/Error: expect(page).to have_content 'http://testbookmark.com'
       expected to find text "http://testbookmark.com" in "Sinatra doesn’t know this ditty. Try this: # in app.rb class BookmarkManager post '/create-new-bookmark' do \"Hello World\" end end"
```
...
> Want to jump in and have a go at the refactor without doing all the other stuff? Check out from [this commit]().
