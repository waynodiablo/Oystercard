...
> If you prefer to work from a code diff, it's [here](https://github.com/sjmog/bookmark_manager/commit/f2ea2c927dc90295e1e52e6d1727f2a1c736626c).
...
> Prefer to read the code directly? You can check out the entire commit [here](https://github.com/sjmog/bookmark_manager/commit/764a5a106ed4236c8abe45fc7dbb496d332023ce).
...
def self.update(id, options)
  connection.exec("UPDATE links SET url = '#{options[:url]}', title = '#{options[:title]}' WHERE id = '#{id}'")
end
```
...
class Link
  def self.find(id)
    result = connection.exec("SELECT * FROM links WHERE id = #{id}")
