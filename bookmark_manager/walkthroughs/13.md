...
[Back to Challenge](../13_wrapping_database_data_in_program_objects.md)

This walkthrough is in 4 parts:

1. Update the `Link.all` method to return an array of `Link` instances, not an array of URLs.
2. Update the `Link` instances expose attributes for `id` and `url`.
3. Add a `title` attribute to Links.
4. Update the rest of our application to use this `link.title` attribute.

## 1. Updating the `Link.all` method to wrap database data in a Link instance
...
class Link
  ...
  def self.all
    result = DatabaseConnection.exec("SELECT * FROM links")
...
## 2. Update the `Link` instances to expose attributes for `id` and `url`.
