...
Now our form submits! But we're just bookmarking to the 'Sinatra doesn't know this ditty' page:
...
What do we do in this route? We'd like something like `Link.delete` to happen – but how will we know which bookmark to delete? We need some way for our form to tell us which 'Delete' button was clicked. Notice that, when we run our test at the moment, the `params` are empty.
...
describe '.delete' do
  it 'deletes a bookmark' do
    Link.create("http://www.makersacademy.com")
    Link.delete(1)
...
> I've decided to make use of Sinatra's [named parameters](http://sinatrarb.com/intro.html) for this route: so visiting `/update-a-bookmark/1` will allow the user to update a bookmark with an ID of 1, and so on.
...
get '/update-a-bookmark/:id' do
  erb :update_a_bookmark
end
```
...
<form action="/update-a-bookmark" method="post">
...
Working backwards, we'd like our `POST /update-a-bookmark` route to do actually update a bookmark. To update a bookmark, we need the ID of the bookmark we're trying to update. To get that, we need the bookmark ID to be submitted by the form. To do that, we need to add a hidden field to the form containing the bookmark ID (like we did with delete). And to do _that_, we need to supply the bookmark ID to the form.

Let's update our `GET /update-a-bookmark/:id` route with the bookmark ID:
...
And update the `POST /update-a-bookmark` route with the SQL required to connect to the database, and update the apppropriate record:
...
To do this, we need to pass more than just `@bookmark_id` to the form. We need to pass a `Link` object that wraps the data for that bookmark. We've got `Link.all` to pull all bookmarks from the database: we need some way of pulling one Link out. Here's how it could work in the controller:

```ruby
# in app.rb

get '/update-a-bookmark/:id' do
  @bookmark = Link.find(params['id'])
  erb :update_a_bookmark
end
```
...
class Link
  def self.find(id)
    ...
    result.map { |bookmark| Link.new(bookmark['id'], bookmark['url'], bookmark['title']) }.first
  end
...
> Just like with `Link.all`, we have to use the ORM pattern to wrap the bookmark data in a useful Ruby object.

Now we can use this `Link` object in our form. We can set the `value` attribute of each input to the current value of the `bookmark`:
