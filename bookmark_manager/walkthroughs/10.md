...
[Back to Challenge](../10_extracting_a_database_setup_object.md)

In this Walkthrough, we will extract a `DatabaseConnection` object, and use it to set up a database connection on-boot. We'll use `DatabaseConnection` in the `Link` class to act on the database, like this:
...
4. Replace `Link` calls to `PG.connect` and `connection` with calls to `DatabaseConnection.query`.
...
## 4. Replace PG with our wrapper in `Link`
