...
Notice how we place parenthesis at the end. Why? This is to communicate to the JavaScript interpreter that we want it to execute this function, as opposed to returning the syntax to us.
...
Armed with this new property, let us create a method to return a boolean check on wether or not the temperature is currently set to the `MINIMUM_TEMPERATURE`:
...
If I were a fortune teller, I would gaze into my crystal ball and tell you that we are going to need a couple of extra constants in our object constructor function:
...
Oh yes, that's right! Le Client wanted us to have a reset button to bring us back to the default temperature! Well, who are we to argue:
...
The last requirement our client has given us, is that we should be able to demonstrate to our user where their energy usage sits, via the medium of colour. Now we stand at the edge of a precipice, for it would be extremely easy right now to make a faux pas and knit our business logic with some presentation logic.

Can you guess why?

Yup, that's right. Colour, is the realm of our visual Designers. Certain colours mean certain things in different countries and cultures. And besides which, just because red, green and amber might be the way you might communicate a three-tier system of energy usage, does not mean that others would agree with you. This is why instead of having our code output `red`, `yellow`, or `green`, , we are going to have our function output `high-usage`, `medium-usage` or `low-usage`. These are still meaningful messages, but have the benefit of being presentation-agnostic, so we can rest easy knowing that the Designers can spend all night tweaking colours and not need to call us Devs in to change the code depending on their visual whims.
